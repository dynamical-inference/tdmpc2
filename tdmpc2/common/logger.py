...
def print_run(cfg):
    ...
    def _pprint(k, v):
        print(prefix + colored(f'{k.capitalize()+":":<15}', color, attrs=attrs),
              _limstr(v))
    ...
    def save(self, step, key='videos/eval_video'):
        if self.enabled and len(self.frames) > 0:
            ...
            return self._wandb.log(
                {
                    key:
                        self._wandb.Video(frames.transpose(0, 3, 1, 2),
                                          fps=self.fps,
                                          format='mp4')
                },
                step=step)
...
class Logger:
    ...
    def pprint_multitask(self, d, cfg):
        ...
        print(
            colored(f'Evaluated agent on {len(cfg.tasks)} tasks:',
                    'yellow',
                    attrs=['bold']))
        dmcontrol_reward = []
        metaworld_reward = []
        ...
        for k, v in d.items():
            ...
            if task in TASK_SET['mt30'] and k.startswith(
                    'episode_reward'):  # DMControl
                dmcontrol_reward.append(v)
                print(colored(f'  {task:<22}\tR: {v:.01f}', 'yellow'))
            elif task in TASK_SET['mt80'] and task not in TASK_SET[
                    'mt30']:  # Meta-World
        ...
        print(
            colored(f'  {"dmcontrol":<22}\tR: {dmcontrol_reward:.01f}',
                    'yellow',
                    attrs=['bold']))
        if cfg.task == 'mt80':
            ...
            print(
                colored(f'  {"metaworld":<22}\tR: {metaworld_reward:.01f}',
                        'yellow',
                        attrs=['bold']))
            print(
                colored(f'  {"metaworld":<22}\tS: {metaworld_success:.02f}',
                        'yellow',
                        attrs=['bold']))
    ...
    def log(self, d, category="train"):
        ...
        if category == "eval" and self._save_csv:
            ...
            pd.DataFrame(np.array(self._eval)).to_csv(self._log_dir /
                                                      "eval.csv",
                                                      header=keys,
                                                      index=None)
