...
def cfg_to_dataclass(cfg, frozen=False):
    ...
    dataclass = dataclasses.make_dataclass(dataclass_name,
                                           fields,
                                           frozen=frozen)
...
def parse_cfg(cfg: OmegaConf) -> OmegaConf:
    ...
    # Convenience
    # cfg.work_dir = Path(
    #     hydra.utils.get_original_cwd()) / 'logs' / cfg.task / str(
    #         cfg.seed) / cfg.exp_name
    ...
    if cfg.multitask:
        ...
        cfg.task_dim = 96 if cfg.task == 'mt80' or cfg.get(
            'model_size', 5) in {1, 317} else 64
    else:
        cfg.task_dim = 0
