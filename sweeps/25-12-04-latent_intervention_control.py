...
# INITIAL_STATES = np.array(angles_radians)

# ============================================================================
# LOAD DATA FOR THE SWEEP
# ============================================================================
path_to_data = '/home/hgf_hmgu/hgf_gib4562/tdmpc2/sweeps/sweep_data/25-12-04-latent_intervention_control'

position_w_global = np.load(os.path.join(path_to_data, 'position_w_global.npy'))

backprop_gradient_linear_position = np.load(
    os.path.join(path_to_data, 'backprop_gradient_linear_position.npy'))

backprop_gradient_nonlinear_position = np.load(
    os.path.join(path_to_data, 'backprop_gradient_nonlinear_position.npy'))

backprop_gradient_nonlinear_action = np.load(
    os.path.join(path_to_data, 'backprop_gradient_nonlinear_action.npy'))

w_action_lasso_results = pickle.load(
    open(os.path.join(path_to_data, 'w_action_lasso_results.pkl'), 'rb'))
...
LR_POSITION_CONFIGS = []
for seed in SEEDS:
    for magnitude in [-1, 1]:  #[-10, -7.5, -5., -2.5, 0, 2.5, 5., 7.5, 10.]:
        LR_POSITION_CONFIGS.append({
            'type': 'directional_addition',
            'location': 'encoder_output',
            'direction': position_w_global,
            #'direction': position_w_global_minus_theta,
            'magnitude': magnitude,
            'seed': seed,
            'direction_str': 'lr_w_global_position'
        })

BACKPROP_GRADIENT_CONFIGS = []
backprop_dirs = {
    # 'backprop_gradient_linear_position':
    #     backprop_gradient_linear_position,
    # 'backprop_gradient_nonlinear_position':
    #     backprop_gradient_nonlinear_position,
    'backprop_gradient_nonlinear_action': backprop_gradient_nonlinear_action,
}
for seed in SEEDS:
    for direction_str, direction in backprop_dirs.items():
        for alpha in [-10, -7.5, -5., -2.5, -1, 0, 1, 2.5, 5., 7.5, 10.]:
            BACKPROP_GRADIENT_CONFIGS.append({
                'type': 'directional_addition',
                'location': 'encoder_output',
                'direction': direction,
                'magnitude': alpha,
...
LASSO_CONFIGS = []
for seed in SEEDS:
    for lasso_alpha in w_action_lasso_results.keys():
        direction = w_action_lasso_results[lasso_alpha]['coef'].squeeze()
        for alpha in [-10, -7.5, -5., -2.5, -1, 0, 1, 2.5, 5., 7.5, 10.]:
...
# for seed in [1]:  # only one seed for now to save time
#     for initial_state in INITIAL_STATES:
#         for alpha in [-10, -1, 0, 1, 10.]:
#             BACKPROP_NONLINEAR_GRADIENT_ROLLOUTS_CONFIGS.append({
#                 'type': 'directional_addition',
#                 'location': 'encoder_output',
#                 'direction': backprop_gradient_nonlinear_position,
#                 'magnitude': alpha,
#                 'hinge_1_value': initial_state,
#                 'seed': seed,
#                 'direction_str': 'backprop_gradient_nonlinear_position'
#             })

INTERVENTION_CONFIGS = LASSO_CONFIGS

#BACKPROP_GRADIENT_CONFIGS
#LR_POSITION_CONFIGS

# Base directory for saving results
date_logged = "25-12-04"
BASE_SAVE_DIR = f'logs/{date_logged}-latent_intervention_control_sweep'
