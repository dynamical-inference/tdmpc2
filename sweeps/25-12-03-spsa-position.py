...
with open(
        "/home/hgf_hmgu/hgf_gib4562/tdmpc2/sweeps/sweep_data/031225-spsa-position/spsa_gradients_x=0.pkl",
        'rb') as f:
    spsa_gradients_x_0 = pickle.load(f)
...
# Example 2: Conditional control - discrete phases
def phase_control(step):
    if step < 150:
        return 7.5  # stabilization
    elif step >= 150:
        return -7.5  # gentle control
    else:
        return 0.0


ALPHAS = [-10, -7.5, -5., -2.5, -1, 0, 1, 2.5, 5., 7.5, 10.]

SPSA_POSITION_CONFIGS = []

for alpha in ALPHAS:
    for gradient_key, gradient in spsa_gradients_x_0.items():
        seed, eps, horizon, random = gradient_key
        SPSA_POSITION_CONFIGS.append({
            ...
            'eps': eps,
            'horizon': str(horizon),
            'random': True if random == 'random' else False,
            'direction': gradient,
        })

INTERVENTION_CONFIGS = SPSA_POSITION_CONFIGS

# Base directory for saving results
BASE_SAVE_DIR = f'logs/25-12-03-spsa-position'
